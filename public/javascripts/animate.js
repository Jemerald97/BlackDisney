...
cOb.c.addEventListener("mouseover", (e)=>{
    let rect = cOb.c.getBoundingClientRect();
    new fw( 700*Math.random(), 600*Math.random(), 125+100*Math.random());
});
}
...
this.draw = ()=>{
    if(!this.boom){
        for(let i = 0; i<=3; i++){
            ...
            new spark(x, y, 3+1*Math.random(),x+(-7+14*Math.random()),y+(-2+4*Math.random()));
        }
        this.update();
    }else{
        for(let i = 0; i<=300; i++){
...
this.update = ()=>{
    ...
    this.cx += 0.1*fps.ads*(this.bx-this.cx) + this.curv*Math.sin((this.cy-this.by)/(cOb.h-this.by)*4*Math.PI);
...
this.draw = ()=>{
    ...
    cOb.ctx.fillStyle = `rgba(255,255,255)`;
    cOb.ctx.beginPath();
    points.forEach((p,i)=>{
